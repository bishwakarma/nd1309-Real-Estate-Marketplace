...
contract Ownable {
    ...
    function transferOwnership(address newOwner) onlyOwner public {
        require(newOwner != address(0), "The address of the new owner should be a real address");
        _owner = newOwner;
...
contract Pausable is Ownable {
    ...
    function setPause(bool pause) onlyOwner public {
        require(_paused != pause, "Contract is already in the same state");
        _paused = pause;
        if(pause) {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != tokenOwner, "Given address is already the owner of the tokenId");
        require(msg.sender == getOwner() || isApprovedForAll(tokenOwner, msg.sender), "Approved address is required to transfer token");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        require(from == ownerOf(tokenId), "From address should be the owner of the specified tokenId");
...
contract CustomERC721Token is ERC721Metadata {
    string private _name = "Hamro Ghar";
    string private _symbol = "HMGR";
    string private _baseTokenURI = "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/";

    constructor()
    ERC721Metadata(_name, _symbol, _baseTokenURI) public {
